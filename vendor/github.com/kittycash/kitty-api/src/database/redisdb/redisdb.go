...
func (r *RedisDB) Add(_ context.Context, entry *iko.KittyEntry) error {
	r.mux.Lock()
	defer r.mux.Unlock()
...
func (r *RedisDB) MultiAdd(ctx context.Context, entries []*iko.KittyEntry) error {
	r.mux.Lock()
	defer r.mux.Unlock()
...
func (r *RedisDB) GetEntryOfID(_ context.Context, kittyID iko.KittyID) (*iko.KittyEntry, error) {
	r.mux.Lock()
	defer r.mux.Unlock()
	...
	return iko.KittyEntryFromJson(entry)
}

func (r *RedisDB) GetEntryOfDNA(_ context.Context, kittyDNA string) (*iko.KittyEntry, error) {
	r.mux.Lock()
	defer r.mux.Unlock()
	...
	return iko.KittyEntryFromJson(raw)
}

func (r *RedisDB) GetEntries(ctx context.Context,
	startIndex, pageSize int,
	filters *database.Filters, sorters *database.Sorters,
) (
	int64, []*iko.KittyEntry, error,
) {
	...
	if sLen > 1 {
		...
		// Set default sorter.
		if err := sorters.Add(database.Sorter("kitty_id")); err != nil {
			return 0, nil, errors.New(
				"internal error")
		}
	}
	...
	for i := 2; i < len(data); i += 2 {
		...
		entry, err := iko.KittyEntryFromJson(entryRaw)
		if err != nil {
			return 0, nil, errors.Errorf(
...
func (r *RedisDB) SetReservationOfEntry(ctx context.Context, kittyID iko.KittyID, reservation string) (*iko.KittyEntry, error) {
	r.mux.Lock()
	defer r.mux.Unlock()
	...
	entry, err := iko.KittyEntryFromJson(rawEntry)
	if err != nil {
		return nil, err
	}
...
func (r *RedisDB) addEntry(key string, raw []byte, entry *iko.KittyEntry) error {
	_, err := r.conn.Do("FT.ADD", r.getAddEntryArgs(key, raw, entry)...)
	return err
}

func (r *RedisDB) getAddEntryArgs(key string, raw []byte, entry *iko.KittyEntry) []interface{} {
	return []interface{}{
		EntryIndexName, key, "1",
...
//func entryKeys(entries []*iko.KittyEntry) []interface{} {
